...
Тема — рассвет: солнце, небо, вода, тепло, ночь перед утром.
...
#: (светлый, средний, тёмный). Средний стоп обязателен: с двумя стопами
#: яркость падает линейно и подложка читается плоским кругом, а не шаром.
ФОН = {
    "common": ("#454c5e", "#343a49", "#232733"),
    "rare": ("#3a83a6", "#2a6480", "#17384b"),
    "epic": ("#8d59cc", "#6a3fa0", "#3f2463"),
    "legend": ("#f0b14a", "#dc7a2f", "#bd4f22"),
}
...
ШАБЛОН = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120"
  ...
  <defs>
    <radialGradient id="ф" cx="34%" cy="26%" r="86%">
      <stop offset="0%" stop-color="{светлый}"/>
      <stop offset="52%" stop-color="{средний}"/>
      <stop offset="100%" stop-color="{тёмный}"/>
    </radialGradient>
    <!-- Тень под мотивом: плоская фигура на плоском фоне не отделяется от
         подложки, и весь набор выглядит наклейками из одного слоя -->
    <filter id="т" x="-20%" y="-20%" width="140%" height="140%">
  ...
  <!-- Верхний блик: свет падает сверху-слева -->
  <ellipse cx="41" cy="28" rx="27" ry="15" fill="#ffffff" opacity=".16"/>
  <!-- Контровой свет снизу: без него низ шара «проваливается» и подложка
       снова читается матовой пуговицей -->
  <path d="M14 78a58 58 0 0092 0" fill="none" stroke="#ffffff"
        stroke-opacity=".09" stroke-width="7" stroke-linecap="round"/>
  <circle cx="60" cy="60" r="58" fill="none" stroke="#ffffff"
          stroke-opacity=".20" stroke-width="2"/>
</svg>
"""
...
def собрать() -> list[dict]:
    ...
    for код, (название, редкость, мотив) in МОТИВЫ.items():
        светлый, средний, тёмный = ФОН[редкость]
        svg = ШАБЛОН.format(
            название=название,
            светлый=светлый,
            средний=средний,
