...
Знак — сплошная искра из tools/brandmark.py.
Геометрия импортируется, а не копируется, поэтому иконка на телефоне
и баннер в боте — буквально одна форма.
...
БАННЕРЫ = [
    ("welcome", 900, 600, "Симп", False),
    ("menu", 900, 450, "Симп", False),
...
def собрать() -> list[str]:
    ...
    for имя, ш, в, надпись, приглушить in БАННЕРЫ:
        ...
        # Радиус описанной окружности: габарит знака = 2 r (см. brandmark.py)
        r = max(28, min(ш, в) // 6)
        cy = в // 2 - (r // 3 if в > 320 else 0)
        нарисовать_знак(im, ш // 2, cy, r, приглушить=приглушить)

        f = _шрифт(max(28, min(ш, в) // 11))
        рамка = d.textbbox((0, 0), надпись, font=f)
        # Нижняя кромка знака — cy + r
        d.text(
            ((ш - (рамка[2] - рамка[0])) // 2, cy + int(r) + max(18, в // 22)),
