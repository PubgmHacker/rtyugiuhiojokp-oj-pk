...
Знак — «стеклянные линзы» из tools/brandmark.py: два объёмных диска,
в пересечении каустика. Геометрия импортируется, а не копируется,
поэтому иконка на телефоне и баннер в боте — буквально одна форма.
