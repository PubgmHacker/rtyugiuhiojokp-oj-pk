...
Знак — «две ауры» из tools/brandmark.py: два пересекающихся круга-ауры,
...
def собрать() -> list[str]:
    ...
    for имя, ш, в, надпись, приглушить in БАННЕРЫ:
        ...
        # Радиус круга знака: полная ширина знака ≈ 2.9 r (см. brandmark.py)
        r = max(24, min(ш, в) // 8)
        ...
        # Нижняя кромка знака — cy + 1.29 r (нижний круг смещён вниз)
        d.text(
            ((ш - (рамка[2] - рамка[0])) // 2, cy + int(r * 1.29) + max(18, в // 22)),
