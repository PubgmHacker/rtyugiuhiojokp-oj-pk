...
Знак марки — «две ауры»: два перекрывающихся круга-ауры, пересечение
загорается светом (см. tools/brandmark.py — там смысл и геометрия).
Иконка — тёмный квадрат в цвете фона приложения со знаком по центру:
на домашнем экране тёмная иконка с двумя цветными кругами различима
с расстояния и не похожа ни на пламя, ни на сердечки соседей по нише.
...
def make_icon(size: int = 1024) -> Image.Image:
    ...
    Радиус круга 0.25 стороны: знак занимает ~72% ширины — достаточно
    крупно для узнавания и с запасом до маски скругления.
    """
    img = Image.new("RGB", (size, size), BG)
    нарисовать_знак(img, size * 0.5, size * 0.5, size * 0.25)
    return img
...
def make_splash(w: int = 2732, h: int = 2732) -> Image.Image:
    ...
    нарисовать_знак(img, w * 0.5, h * 0.5, min(w, h) * 0.055)
    return img
...
def main() -> None:
    ...
    # у уменьшенной копии линза пересечения замыливается в один пиксель
    fav_sizes = [(16, 16), (32, 32), (48, 48), (64, 64)]
    fav = Image.new("RGB", (64, 64), BG)
    нарисовать_знак(fav, 32, 32, 64 * 0.30)
