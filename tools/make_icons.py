...
Знак марки — «стеклянные линзы»: два объёмных диска, пересечение —
каустика мэтча (см. tools/brandmark.py — там смысл и геометрия).
Иконка — тёмный квадрат в цвете фона приложения со знаком по центру:
на домашнем экране тёмная иконка с двумя цветными линзами различима
...
def make_icon(size: int = 1024) -> Image.Image:
    ...
    Радиус круга 0.23 стороны: при габарите знака ≈ 3.16 R это ~73%
    ширины — крупно для узнавания и с запасом до маски скругления.
    """
    img = Image.new("RGB", (size, size), BG)
    нарисовать_знак(img, size * 0.5, size * 0.5, size * 0.23)
    return img
