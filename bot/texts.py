...
def reg_step(step: str) -> str:
    steps = {
        ...
        "photo": (
            "Пришлите фото 📸\n\n"
            "<i>Одно обязательно, можно до пяти — первое станет главным "
            "в анкете. Можно добавить и видео до 20 МБ (до трёх).</i>"
        ),
        "bio": (
            "Пара слов о себе ✍️\n\n"
...
REG_PHOTO_ADDED = "Фото добавлено ({count}/5). Пришлите ещё или нажмите «Готово»."
REG_PHOTO_LIMIT = "Пять фото — максимум. Идём дальше 👇"
...
# ── Видео в анкете ──────────────────────────────────────────────
# Видео — дополнение к фото, не замена: хотя бы одно фото обязательно,
# и именно фото проходят гейт «живой человек».
REG_VIDEO_ADDED = (
    "Видео добавлено ({count}/{limit}) 🎬 Пришлите ещё фото или видео — "
    "или нажмите «Готово»."
)
REG_VIDEO_LIMIT = "Больше {limit} видео в анкету не поместится 🙂 Пришлите фото или нажмите «Готово»."
REG_VIDEO_TOO_BIG = (
    "Это видео больше {limit_mb} МБ — такое бот принять не может 😔\n\n"
    "Обрежьте ролик или сожмите его и пришлите ещё раз."
)
REG_VIDEO_REJECTED = "Это видео не подходит: {reason}\n\nПришлите, пожалуйста, другое."
REG_VIDEO_FETCH_FAILED = (
    "Не получилось скачать видео у Telegram 😔 Пришлите его ещё раз."
)
# Без превью модерации не на что смотреть — а без проверки мы видео
# не публикуем (то же fail-closed правило, что у фото).
REG_VIDEO_NO_PREVIEW = (
    "У этого видео нет превью, и проверить его мы не можем — а без проверки "
    "видео не публикуем. Пришлите ролик обычным видео (не файлом) 🙏"
)
REG_VIDEO_MOD_UNAVAILABLE = (
    "Проверка видео сейчас недоступна, а без неё мы ролики не публикуем. "
    "Попробуйте через пару минут 🙏"
)
...
#: текстовая, а сами наклейки — картинки WebP для веба. Поэтому рядом с именем
...
UNKNOWN_MESSAGE = (
    "Не понял сообщение 🤔\n\n"
    "Используйте кнопки меню или /help — там собраны все возможности."
)
