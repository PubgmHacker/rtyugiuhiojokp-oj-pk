...
(две платные), три кейса с наклейками-значками на фото анкеты («Керопи»,
«Star Rail», «Хеллоуин») и обложками карточки — без расходников, три
...
пересекался с целью. Чужие фото ловятся сверкой лиц: три несовпадения за
30 дней — автобан. Забаненному бот показывает экран блокировки с платной
досрочной разблокировкой (`UNBAN_PRICE_RUB`, по умолчанию 349 ₽, Telegram
Stars); возврат Stars возвращает бан на место. Блокировка пользователя
необратима: пара исчезает
из выдачи друг друга навсегда. Фото перекодируются перед загрузкой —
EXIF с GPS-координатами не попадает в публичное хранилище. Регистрация с 18 лет (рейтинг App Store — отдельно, см. APPSTORE.md).
...
Второй принцип: цветом обозначаем действие и статус, поверхности
держим глухими и почти монохромными — база лишь подкрашена в тон
акцента. Внимание должно доставаться фотографиям людей, а не
интерфейсу.

- Базовая схема — «Туманность»: фон `#080615`, поверхности `#140f2e` /
  `#1c1540` / `#281d58` — глубокий фиолетово-чёрный, тон один, шаги
  только по светлоте
- Акцент `#a855f7` — один фиолет на все главные действия (5.1:1 на
  фоне; белый глиф на заливке 4.0:1 — кнопки набраны крупно и жирно,
  мелкий текст на акценте ставится на `accent-deep` `#7c2fd4`, 6.5:1).
  Кнопка-факел `--gradient-torch`: `accent-soft → accent → accent-deep`,
  без цветных свечений и теней
- Текст четырьмя ступенями: `#fafbfc` / `#cdc7e2` / `#a29bbf` /
  `#8c86a8`, все проходят AA на базовом фоне (самая слабая — 5.8:1)
- Семантика: лайк `#34d399`, опасность `#ff7a1a` (тёплая на холодной
  базе — «ошибка» не читается как главное действие),
  предупреждение `#f0b429`, верификация `#58a6ff`
- Семь схем оформления: базовая Туманность живёт в `@theme` без
  атрибута, остальные — Классика (прежняя малиновая база), Полночь,
  Графит, День, Сепия и платное Золото — через `:root[data-appearance=…]`.
  Схема переопределяет только токены, экраны не правятся; выбор хранится
  и на сервере (`profile.app_theme`), чтобы переезжал между устройствами
...
- Фон живой (`components/LivingBackground.tsx`): три гауссовых орба и
  светлая искра в цветах схемы (`--orb-1..3`, `--spark` на `<html>`),
  сверху зерно и скрим. Анимируются только `translate` / `scale` /
  `rotate` — без `filter: blur()` и `mix-blend-mode`, iOS их
  ре-растеризует. Замирает по `prefers-reduced-motion` и тумблеру «Живое
  движение» (`data-motion="still"`), на светлых схемах выключен
  (`data-living="off"`). Базовая заливка живёт на `html`, `body` и
  `#root` прозрачны — иначе фон предка закрыл бы слой с `z-index: -1`
