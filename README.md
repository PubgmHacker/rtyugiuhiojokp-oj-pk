...
- Знак марки — «две ауры»: два перекрывающихся круга разных оттенков,
  в пересечении свет (`tools/brandmark.py` — геометрия для PIL;
  `web/src/components/BrandMark.tsx` — тот же знак в интерфейсе).
  Смысл из `web/src/lib/aura.ts`: у каждого своя аура, мэтч — место
