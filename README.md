...
EXIF с GPS-координатами не попадает в публичное хранилище. Регистрация с 16 лет (рейтинг App Store — отдельно, см. APPSTORE.md).
...
не выводится**. Никаких метафор — ни в палитре, ни в именах утилит.
Второй принцип: цветом обозначаем только действие и статус, всё
остальное — оттенки серого. Внимание должно доставаться фотографиям
людей, а не интерфейсу.
...
- Знак марки — одна искра мэтча с каплей внутри: градиент двух аур на
  одной форме (`tools/brandmark.py` — геометрия для PIL;
  `web/src/components/BrandMark.tsx` — тот же знак в интерфейсе).
  Смысл из `web/src/lib/aura.ts`: у каждого своя аура, мэтч — вспышка
  встречи. Одна форма на иконке, сплэше, favicon, баннерах бота и
  og-превью; читается и в 16px, и в 1024px
