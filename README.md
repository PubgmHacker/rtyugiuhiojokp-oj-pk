...
Принцип: цветом обозначаем только действие и статус, всё остальное —
оттенки серого. Внимание должно доставаться фотографиям людей, а не
интерфейсу, поэтому цветных свечений и многоцветных заливок нет.

- Фон `#100e0e`, поверхности `#191516` / `#201b1c` / `#292224` — тёплый
  сливовый подтон (345°), «ночь перед рассветом». Холодные серые остались
  от прежнего индиго и после смены акцента на тёплый спорили с ним:
  интерфейс выглядел синим, а кнопки — чужими на нём. Чистая терракота
  читалась бы грязно-коричневой, поэтому подтон сливовый, а не бурый
- Акцент `#e14a35` — один закатный цвет на все главные действия
  (4.80:1 на фоне, белый глиф на заливке 4.01:1)
- Текст четырьмя ступенями: `#fbf6f7` / `#d6c4c8` / `#a8979b` / `#918286`
  (18.0 / 11.5 / 6.9 / 5.3 к фону). Подтон тот же сливовый: холодно-серый
  текст на тёплом фоне выглядит выцветшим
- Семантика: лайк `#34d399`, пропуск `#ff5c8a`, верификация `#58a6ff`.
  Лайк и пропуск обязаны различаться цветом — промах стоит мэтча
- Фон живой: два очень слабых пятна (0.06 и 0.05) под контентом,
  закреплены за вьюпортом, анимация 40 с и глохнет по
