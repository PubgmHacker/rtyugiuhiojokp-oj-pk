...
class UserProfile(BaseModel):
    ...
    #: Аккаунт команды Симпа (роль admin/owner). Считается из роли на сервере
    #: — см. utils.официальный. Клиент по нему рисует золотой бейдж, чтобы
    #: письмо «из поддержки» нельзя было подделать именем.
...
class ChatAttachment(BaseModel):
    """Одно вложение переписки — строка витрины «Вложения».

    Главное поле здесь — `message_id`: витрина не открывает файл в отрыве
    от разговора, а прыгает к самому сообщению в истории. Поэтому у всех
    пяти видов вложений одна форма, а не пять разных.
    """

    message_id: str
    #: photo | reel | video_note | voice | link
    kind: str
    url: str = ""
    #: Постер: кадр кружка или обложка ролика. У фото совпадает с url.
    poster: str = ""
    #: Фигура видеокружка (circle, heart, star…) — витрина рисует ту же.
    shape: str = ""
    #: Секунды: голосовое и кружок.
    duration: Optional[int] = None
    #: Волна голосового — те же цифры 0–9, что в самом сообщении.
    waveform: str = ""
    #: Домен ссылки — заголовок строки во вкладке «Ссылки».
    host: str = ""
    #: Отправлено мной, а не собеседником.
    from_me: bool = False
    created_at: Optional[datetime] = None


class ChatAttachments(BaseModel):
    """Что переслали друг другу за всю переписку.

    Счётчики считаются агрегатом по ВСЕЙ переписке, списки — только
    последней страницей: цифра сверху обязана быть честной, даже когда
    лента под ней обрезана.
    """
    ...
    video_notes: int = 0
    voices: int = 0
    #: Суммарная длительность голосовых, секунды: «7 голосовых · 4 мин».
    voice_seconds: int = 0
    links: int = 0
    #: Фото, ролики и кружки одной лентой — свежие первыми, как в Telegram.
    media: list[ChatAttachment] = Field(default_factory=list)
    voice: list[ChatAttachment] = Field(default_factory=list)
    link: list[ChatAttachment] = Field(default_factory=list)
