...
LIMITS: list[tuple[str, str, int, int]] = [
    ...
    # Перебор промокодов — тот же жанр: код короткий и человекочитаемый, а
    # выигрыш — бесплатная подписка. Свой лимит попыток, как у кодов привязки,
    # в промокодах не заведён, так что этот — единственный.
    ("/api/promo/activate", "POST", 10, 3600),
