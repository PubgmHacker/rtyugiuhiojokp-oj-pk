...
async def test_путь_от_деки_до_мэтча(клиент, живая_база):
    ...
    # Человек только что был в сети, но незнакомцу этого не показываем:
    # статус присутствия — только внутри мэтча
    assert дека[0]["is_online"] is False, "«в сети» утекло незнакомцу"
