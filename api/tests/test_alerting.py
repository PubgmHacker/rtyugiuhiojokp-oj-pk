...
def test_с_dsn_но_без_установленного_пакета_init_не_роняет_приложение(caplog):
    """Пакет sentry-sdk может быть не установлен — это осознанно необязательная
    зависимость. init с DSN должен просто предупредить, а не бросить исключение."""
    from services import alerting
    ...
    # В тестовом окружении sentry-sdk не установлен — алертинг остался выключен
    assert alerting._enabled is False
