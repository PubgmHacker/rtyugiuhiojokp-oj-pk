...
/**
 * Заголовок месяца — группы в витрине вложений. Год ставим только чужой: в
 * переписке этого года «август» читается быстрее, а в прошлогодней без года
 * августы разных лет сливаются в один.
 */
export function форматМесяца(дата: Date, язык: Язык, сГодом: boolean): string {
  return форматтер(язык, {
    month: "long",
    ...(сГодом ? { year: "numeric" as const } : {}),
  }).format(дата);
