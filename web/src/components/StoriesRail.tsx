 ...
 *
 * Кольцо — одно фирменное на всех (tone="brand"). Цветом человека красить
 * пробовали: в полосе кружки идут подряд, и восемь оттенков ауры давали
 * радугу поперёк экрана, при этом имя напечатано под каждым кружком —
 * оттенок никого не опознаёт, он только шумит.
 *
 * Просмотренные не убираем, а меняем кольцо на волосяную линию: исчезающие
 * элементы дёргают полосу под пальцем, и человек теряет место, где
 * остановился, а разница «градиент против тонкой линии» видна и боком глаза.
 */
      ...
      {viewing && (
        <StoryViewer
          ...
          avatar={
            viewing === user?.id
              ? user?.photos?.[0] || null
              : feed.authors.find((a) => a.user_id === viewing)?.avatar || null
          }
          onClose={() => {
            setViewing(null);
